...
- 做了什么：在 `paper/GLOSSARY.md` 新建论文专业术语表（中英对照 + 名词解释 + 翻译规则），要求论文所有术语 / 专业名词详细登记；同步更新 `paper/README.md`、`paper/en/README.md` 引用术语表；将协作契约核心原则强化为「先质疑，不盲目认可与服从」（`AGENTS.md` 核心原则 + `README.md` 工作规则第 8 条）；新增质量标准章节（设计先评审后实施、不轻易降级、论文以提升代替降级、敢于大改、力争最好）；新增 Git 与版本纪律（阶段性同步、先备份再推进、版本隔离、复制后自包含不跨版本引用）。
